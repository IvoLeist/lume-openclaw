...
#
# Usage:
#   ./scripts/launch-vm-and-gateway.sh
#   VM_USER=myuser ./scripts/launch-vm-and-gateway.sh
#
set -euo pipefail
...
if ! command -v lume &>/dev/null; then
  echo "Lume is not installed. Run ./scripts/install-lume.sh first."
  exit 1
fi
...
if [[ -z "$IP" ]]; then
  ...
  if ! ssh_ok "$IP"; then
    echo "SSH not ready. You can try again in a minute: ./scripts/openclaw-gateway.sh"
    exit 1
  fi
else
...
exec "$SCRIPT_DIR/openclaw-gateway.sh"
